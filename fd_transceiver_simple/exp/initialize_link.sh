...
ssh $1@console.grid.orbit-lab.org << 'END_CONSOLE_SSH'
    ...
    omf load -i tingjunchen-node-node11-10.grid.orbit-lab.org-2017-11-07-12-33-57.ndz -t node$NODEX-$NODEY,node13-8 -o 300
    ...
    omf tell -a on -t node$NODEX-$NODEY,node13-8

    # Farewell
    echo "Initialization process finished."
    echo "Use node"$NODEX"-"$NODEY" as the FD node and node13-8 as the HD node."
