...
class sic(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    ...
    def __init__(self, frame_size=5000, preamble_size=600, k=30, frames=500, error_report=1, warning_report=0, information_report=0):  # only default arguments here
    ...
    def work(self, input_items, output_items):
        ...
        if in_size >= self.fs + 2 * self.k and not(self.first):  # not the first frame
            if self.fc % self.f == 0:
                # find h
                h_time = time.time()
                self.h = find_h_np(self.k, self.p, self.in0_buff[: self.k + self.fs], self.in1_buff[: self.k + self.fs], self.wr)
            ...
            # calc the output
            data_time = time.time()
            A = build_A(self.in1_buff[: self.fs + 2 * self.k], self.k)
            ...
            if self.fc % self.f == 0:
                # find h
                h_time = time.time()
                self.h = find_h_np(self.k, self.p, self.in0_buff[: self.fs], self.in1_buff[: self.fs], self.wr)
                h_time = time.time() - h_time
                if h_time > self.h_time and self.ir:
                    self.h_time = h_time
                    print 'h time', self.h_time
            # calc the output
            data_time = time.time()
            A = build_A(self.in1_buff[: self.fs + self.k], self.k)
            data_time = time.time() - data_time
            if data_time > self.data_time and self.ir:
                self.data_time = data_time
                print 'data time', self.data_time
...
def build_A(x, k):
    n = len(x) - 2 * k
    A = np.zeros((n, 2 * k), dtype=np.complex64)
