...
packet_encap2_impl(bool debug,
      ...
      d_sig_pilot_duplicates = 10;

      for (int idx_duplicate = 0; idx_duplicate<d_sig_pilot_duplicates; idx_duplicate++) {
        for (int idx_sig_pilot = 0; idx_sig_pilot<80; idx_sig_pilot++) {
          d_sig_pilot[idx_sig_pilot+80*idx_duplicate] = d_sig_pilot[idx_sig_pilot];
        }
      }

      for (int idx_sig_pilot = 0; idx_sig_pilot<80*d_sig_pilot_duplicates; idx_sig_pilot++) {
...
int work (int noutput_items, gr_vector_int& ninput_items,
  ...
  */
  std::memset(out, 0x00, sizeof(gr_complex)*(ninput_items[0] + 2*d_sig_pilot_duplicates*d_sig_pilot_len + d_pad_front + d_pad_tail));
  std::memcpy(out + d_pad_front + d_sig_pilot_pos*d_sig_pilot_len*d_sig_pilot_duplicates, d_sig_pilot, sizeof(gr_complex)*d_sig_pilot_len*d_sig_pilot_duplicates);
  std::memcpy(out + d_pad_front + 2*d_sig_pilot_duplicates*d_sig_pilot_len, in, sizeof(gr_complex)*ninput_items[0]);

  // std::cout << "Pad position: " << d_sig_pilot_pos*d_sig_pilot_len << std::endl;

  int produced = ninput_items[0] + 2*d_sig_pilot_duplicates*d_sig_pilot_len + d_pad_front + d_pad_tail;
